...
if ($nick != "") {
		...
		foreach ($rs2 as $t => $fila2) {
			if ($grup == $fila2["grupo"]) {
				...
				if ($cuerp == false or $cuerp == "") {
					echo "<li><a onclick='location.href=\"../".$fila2["contenido"]."\"' terget='cuerpo'style='cursor:pointer;'>".$fila2["opcion"]."</a></li>";
				}
				else
					echo "<li><a onclick='location.href=\"".$fila2["contenido"]."\"' terget='cuerpo'style='cursor:pointer;'>".$fila2["opcion"]."</a></li>";
			}
	...
	echo "/ul";
	if ($cuerp == false) {
		echo "<a onclick='location.href=\"../claves/\"'><input type='button' name='accion' value='Cerrar Sesion' style='cursor:pointer;border-radius:10px;font-weigth:bold;height:25px;' class='boton_cerrar'></a>";
	}else{
		echo "<a onclick='location.href=\"claves/\"'><input type='button' name='accion' value='Cerrar Sesion' style='cursor:pointer;border-radius:10px;font-weigth:bold;height:25px;' class='boton_cerrar'></a>";
	}
	echo 
